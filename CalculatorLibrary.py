...
class CalculatorLibrary(object):
    ...
    def push_buttons_data(self):
    	"""Pushes the buttons but the data is created on the fly, not taken from any file
	...
	print num2
	print operator
        self.push_button(str(num1))
        self.push_button(str(operator))
        self.push_button(str(num2))
        self.push_button('=')
        expression = str(num1) + str(operator) + str(num2)
        expected_result = str(eval(expression))
        print self._result
        print expected_result
        if self._result != expected_result:
            raise AssertionError('%s != %s' % (self._result, expected_result))